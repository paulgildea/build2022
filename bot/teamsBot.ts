...
import inventoryItemCard from "./adaptiveCards/inventoryItem.json"
import inventoryData from "./adaptiveCards/inventoryItem.data.json"
...
export class TeamsBot extends TeamsActivityHandler {
  ...
  public async handleTeamsAppBasedLinkQuery(context: TurnContext, query: any): Promise<any> {
    const preview = CardFactory.thumbnailCard(inventoryData.title, inventoryData.description, [inventoryData.image]);

    const card = AdaptiveCards.declare(inventoryItemCard).render(inventoryData);
    const factoryCard = CardFactory.adaptiveCard(card);
    const attachment = {
      contentType: factoryCard.contentType,
      content: factoryCard.content,
      preview: preview
    };
